...
export class Worker {
   ...
   */
  public async getMessagewithUids(uids:string): Promise<string[]> {
   ...
   */
   public async listNamespaces(): Promise<IMailbox[]> {
    console.log("IMAP.Worker.listNamespaces()");
    const client: any = await this.connectToServer();
    // noinspection TypeScriptValidateJSTypes
    const namespaceObjects:IMailbox[] = await client.listNamespaces();
    await client.close();
   ...
   */
   public async searchUnseenMessage(): Promise<number[]> {
    console.log("IMAP.Worker.searchUnseenMessage()");
    const client: any = await this.connectToServer();
    // noinspection TypeScriptValidateJSTypes
    const namespaceObjects:number[] = await client.search('INBOX', {unseen: true});
    await client.close();
    return namespaceObjects;
